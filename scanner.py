...
def send_email(df):
    ...
    if df.empty:
        subject = f"VCP Scan {datetime.now().strftime('%d-%b')}: No setups"
        body = "<p>No VCP 3/4 or 4/4 setups found today.</p>"
    else:
        df = df.sort_values(['Scan', 'Stock'], ascending=[False, True])
        subject = f"VCP Stocks {datetime.now().strftime('%d-%b')}: {len(df)} setups"
        html_table = df.to_html(index=False, border=1, justify='center')
        body = f"""
        <h3>Daily VCP Scan - Nifty 500</h3>
        <p><b>Buy Trigger:</b> Price closes above Entry with Volume > 1.5x 50-day avg</p>
        <p><b>Risk:</b> SL is pattern low. Exit if it hits. T1=2R, T2=3R</p>
        {html_table}
        <p><i>Automated scan. Not investment advice.</i></p>
        """
